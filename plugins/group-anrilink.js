...
// Handler follows your original pattern
let handler = m => m
...
handler.before = async function (m, { conn, isBotAdmin, isAdmin }) {
  try {
    // Basic checks (your original style)
    ...
    await m.reply(
      ...
      `Sender: *${tag}*\n` +
      `Warning: *${groupData.users[sender].warn}/5*\n\n` +
      `> Sending links again = auto kick`,
      null,
      { mentions: [sender] }
    )
    ...
    if (groupData.users[sender].warn >= 5) {
      await m.reply(
        `🚫 *ANTI LINK*\n\n` +
        `*${tag}* has reached 5 warnings and will be removed.`,
        null,
        { mentions: [sender] }
      )
