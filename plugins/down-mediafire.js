...
let handler = async (m, { conn, text, usedPrefix, command, isPrems }) => {
  ...
  if (!mediaRegex.test(text)) return m.reply('Invalid link! Make sure the MediaFire link is correct.');
...
async function mediafire(url) {
  ...
  if (json.response.result !== 'Success') throw 'Failed to get file info.';
